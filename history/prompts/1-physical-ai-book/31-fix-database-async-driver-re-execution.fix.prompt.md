---
id: 31
title: Fix Database Async Driver (Re-execution)
stage: fix
date: 2025-12-15
surface: agent
...
The `DATABASE_URL` in `backend/.env.example` has been updated to use the `postgresql+asyncpg://` format, resolving the `sqlalchemy.exc.InvalidRequestError`. This is a re-execution of a previously completed fix.
...
- 🧠 Reflection: This task was previously completed. The user re-issued the same request. The same fix has been applied and communicated. Manual verification steps are crucial for the user to confirm the resolution.
