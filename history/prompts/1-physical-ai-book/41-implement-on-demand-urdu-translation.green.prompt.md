...
files:
 ...
 - frontend-docusaurus/website/src/theme/DocItem/index.tsx
tests:
 - npm run typecheck
 - npm run build
---
...
2.  **Frontend Per-Chapter Button:**
    *   Instead of modifying the global header, I modified the `DocItemLayout` component by ejecting the original `DocItem` component, which wraps each chapter's content.
    *   Using `useEffect`, `useState`, and `useDocusaurusContext` hooks, the component now dynamically creates a "Translate to Urdu" button and injects it directly after the `<h1>` chapter title on each doc page. This ensures the button is specific to each chapter.
...
The implementation meets all the specified requirements for a per-chapter, on-demand translation feature without modifying any existing authentication or global components. The frontend now builds successfully.

## Outcome

- ✅ Impact: Successfully implemented a new feature for on-demand Urdu translation of book chapters. A 'Translate to Urdu' button now appears on each chapter page, which, when clicked, uses a new Gemini-powered backend agent to translate the content in-place. The frontend builds successfully.
...
- 📁 Files: Created a new Python translation agent and a FastAPI endpoint. Modified `requirements.txt` and `main.py` accordingly. On the frontend, swizzled `DocItem` and modified `DocItem/Layout/index.tsx` to inject a per-chapter button and handle the translation workflow, including fixing a type error during the process.
...
- 🧠 Reflection: The task was complex, involving coordination between frontend and backend. Swizzling `DocItem` provided the necessary control for button placement. Fixing the type error in the Docusaurus theme file was critical to ensure a clean build.
